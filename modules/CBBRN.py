...
LastEditTime: 2024-06-07 21:19:03
FilePath: \学位论文代码speechSR\modules\CBBRN.py
Description: 第三章模型
...
'''
description: 测试用 LocalTransforme
return {*}
'''
...
class ResidualUnit(nn.Module):
    ...
    def __init__(self, channels, nl, dilation, bias=False, norm: str = 'weight_norm', norm_params: tp.Dict[str, tp.Any] = {}, causal: bool = False,
        ...
        self.pointwise_conv = SConv1d(channels, channels, kernel_size=1, norm=norm, norm_kwargs=norm_params,
                causal=causal, pad_mode=pad_mode)
        self.nl = nl
        self.rnn = nn.GRU(batch_first=True, bidirectional=True, hidden_size=channels,input_size=channels)
        self.linear = nn.Linear(channels * 2,channels)
        self.bn = nn.BatchNorm1d(channels)
        self.nl2 = nn.LeakyReLU(negative_slope=0.01)
...
class EncBlock1(nn.Module):
    ...
    def __init__(self, out_channels, stride, nl, bias=False, norm: str = 'weight_norm', norm_params: tp.Dict[str, tp.Any] = {}, causal: bool = False,
        ...
        self.residuals = nn.Sequential(
            ResidualUnit(channels=out_channels // 2, nl=nl, dilation=1, norm=norm, norm_params=norm_params,
                                      causal=causal, pad_mode=pad_mode),
            ResidualUnit(channels=out_channels // 2, nl=nl, dilation=4, norm=norm, norm_params=norm_params,
                                      causal=causal, pad_mode=pad_mode),
            ResidualUnit(channels=out_channels // 2, nl=nl, dilation=12, norm=norm, norm_params=norm_params,
                                      causal=causal, pad_mode=pad_mode))
        self.conv = SConv1d(out_channels // 2, out_channels,
                kernel_size=2 * stride, stride=stride,
                norm=norm, norm_kwargs=norm_params,
...
class DecBlock1(nn.Module):
    ...
    def __init__(self, out_channels, stride, nl, bias=False, norm: str = 'weight_norm', norm_params: tp.Dict[str, tp.Any] = {}, causal: bool = False,
        ...
        self.residuals = nn.Sequential(
            ResidualUnit(channels=out_channels, nl=nl, dilation=1, norm=norm, norm_params=norm_params,
                                      causal=causal, pad_mode=pad_mode),
            ResidualUnit(channels=out_channels, nl=nl, dilation=4, norm=norm, norm_params=norm_params,
                                      causal=causal, pad_mode=pad_mode),
            ResidualUnit(channels=out_channels, nl=nl, dilation=12, norm=norm, norm_params=norm_params,
                                      causal=causal, pad_mode=pad_mode))
...
class seanetbwechannel(nn.Module):

    def __init__(self, norm: str = 'weight_norm', norm_params: tp.Dict[str, tp.Any] = {}, causal: bool = False, # False
                 pad_mode: str = 'reflect', trim_right_ratio: float = 1.0,stride = [2,4,8],first_kernal = 7, latent_kernel = 7, channel = 48):
        """
        Generator of seanet
...
class seanetbwediliation(nn.Module):
    ...
    def __init__(self, norm: str = 'weight_norm', norm_params: tp.Dict[str, tp.Any] = {}, causal: bool = False, # False
        ...
        self.encoder_blocks = nn.ModuleList(
            [EncBlock1(out_channels=channel * 2, stride=stride[0], nl=self.nl, norm=norm, norm_params=norm_params,
                                      causal=causal, pad_mode=pad_mode),
             EncBlock1(out_channels=channel * 4, stride=stride[1], nl=self.nl, norm=norm, norm_params=norm_params,
                                      causal=causal, pad_mode=pad_mode),
             EncBlock1(out_channels=channel * 8, stride=stride[2], nl=self.nl, norm=norm, norm_params=norm_params,
                                      causal=causal, pad_mode=pad_mode)])
        ...
        self.decoder_blocks = nn.ModuleList(
            [
             DecBlock1(out_channels=channel * 4, stride=stride[2], nl=self.nl,
                                 norm=norm, norm_params=norm_params,
                                 causal=causal, trim_right_ratio=trim_right_ratio),
             DecBlock1(out_channels=channel * 2, stride=stride[1], nl=self.nl,
                                 norm=norm, norm_params=norm_params,
                                 causal=causal, trim_right_ratio=trim_right_ratio),
             DecBlock1(out_channels=channel, stride=stride[0], nl=self.nl,
                                 norm=norm, norm_params=norm_params,
                                 causal=causal, trim_right_ratio=trim_right_ratio)])
...
class seanetbwestride(nn.Module):

    def __init__(self, norm: str = 'weight_norm', norm_params: tp.Dict[str, tp.Any] = {}, causal: bool = False, # False
                 pad_mode: str = 'reflect', trim_right_ratio: float = 1.0,stride = [2,8,8],first_kernal = 7, latent_kernel = 7, channel = 16):
        """
        Generator of seanet
...
if __name__ == '__main__':
    ...
    print('flops: %.2f M, params: %.2f M' % (flops / 1e6, params / 1e6)) # seanetbwe flops: 8513.63 M, params: 0.95 M