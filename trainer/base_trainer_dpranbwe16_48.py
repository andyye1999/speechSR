...
LastEditTime: 2024-04-23 16:29:33
FilePath: \lastdance\trainer\base_trainer_dpranbwe16_48.py
Description: 第四章模型的训练配置
...
from model.dparn import DPModel
...
class BaseTrainer:
    def __init__(self,
        ...
        # self.discriminator = MultiScaleSTFTDiscriminator(filters=32).to(self.device)
        self.discriminator = MultiScaleSTFTDiscriminator(filters=32,n_ffts = [256, 512, 1024, 2048],hop_lengths = [64, 128, 256, 512],win_lengths = [256, 512, 1024, 2048]).to(self.device)
        # weights = torch.load('F:\\yhc\\bone\\generator.ckpt')
        # self.model.load_state_dict(weights)
        self.loss1 = cntloss() # 没用 测试用的
        self.loss2 = SpectralReconstructionLoss(sr=48000, reduction='mean', device=self.device,) # 多尺度melloss
        self.model = DPModel(model_type='DPRAN', device=self.device).to(self.device) # 模型
    ...
    def compute_ema_lambda_adaptive(self, loss_recons, loss_adv):
        ...
        grads_recons = \
        torch.autograd.grad(outputs=loss_recons, inputs=self.model.last_conv.weight, retain_graph=True)[0] # 梯度范数
        grads_adv = \
        torch.autograd.grad(outputs=loss_adv, inputs=self.model.last_conv.weight, retain_graph=True)[0] # 梯度范数
