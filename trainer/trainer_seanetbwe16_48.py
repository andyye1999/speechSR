...
LastEditTime: 2024-06-07 21:14:13
FilePath: \学位论文代码speechSR\trainer\trainer_seanetbwe16_48.py
Description: 第三章模型训练过程
...
from trainer.base_trainer_seanetbwe16_48 import BaseTrainer
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (mixture, clean, filename) in enumerate(self.train_data_loader):
            ...
            for optimizer_idx in [0, 1]:  # 生成器和判别器同时训练
                if optimizer_idx == 0:
                    loss_rec_time = F.l1_loss(enhanced_speech, clean, reduction="mean") # 时域重建损失
                    loss_rec_mel = self.loss2(enhanced_speech, clean) # mel频域重建损失
                    lamda1 = 1
                    lamda2 = 10
                    loss_G = lamda1 * loss_rec_time + lamda2 * loss_rec_mel # 权重
                    ...
                    # lamda = self.compute_ema_lambda_adaptive(loss_fm,loss_adv)  平衡器 在seanet使用
                    lamda3 = 1
                    lamda4 = 0
                    loss_G += lamda3 * loss_fm +  lamda4 * loss_adv  # 生成器损失函数
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (mixture, clean, name ) in enumerate(self.validation_data_loader):
            ...
            if mixture.size(-1) % sample_length != 0: # 截断
            ...
            enhanced_chunks = []
            for chunk in mixture_chunks: # 通过模型
            ...
            snr_j = SNR(enhanced,clean) # 计算SNR
            base_snr_j = SNR(mixture,clean)
        ...
        score = 1 / get_metrics_ave(lsd_c_e) #计算分数
        return score
